...
class AvailabilityProvider with ChangeNotifier {
  ...
  setRangeHours(TimeOfDay startHourProp, TimeOfDay endHourProp, List<Assignment> assignments) {
    startHour = startHourProp;
    endHour = endHourProp;

    if (startDate != null && endDate != null && assignments != null) {
      setSelectedDates(startDate, endDate, assignments);
    }
    notifyListeners();
  }

  setSelectedDates(DateTime date1, DateTime date2, List<Assignment> assignments) {
    List<int> checks = [];
    ...
    if (date1 != null) {
      ...
      while (rangeDate.isBefore(computedDate2) || rangeDate.isAtSameMomentAs(computedDate2)) {
        checks.add(isDayAvailable(rangeDate, assignments, startHour, endHour));
